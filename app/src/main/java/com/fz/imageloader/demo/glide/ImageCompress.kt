...
object ImageCompress {
     ...
     */
    fun compressImageDataWithLongWidth(
        context: Context,
        rawData: ByteArray,
        limitLongWidth: Int
    ): ByteArray? {
        ...
        val longSideWidth = Math.max(imageWidth, imageHeight)
        ...
        if (format == ImageFormat.GIF) {
            // 压缩 Gif 分辨率太大编码时容易崩溃
            return compressGifDataWithLongWidth(context, rawData, Math.max(512, longSideWidth))
        } else {
     ...
     */
    fun compressImageDataWithSize(
        context: Context,
        rawData: ByteArray,
        limitDataSize: Int
    ): ByteArray? {
        dLog { "rawData.size:" + rawData.size + ",limitDataSize:" + limitDataSize }
        ...
        if (format == ImageFormat.GIF) {
            ...
            dLog { "rawData.size:" + rawData.size + ",sampleCount:" + sampleCount }
            val data = compressGifDataWithSampleCount(context, resultData, sampleCount)
            dLog { "data.size:" + data?.size + ",sampleCount:" + sampleCount }
            if (data != null) {
                resultData = data
            } else {
                return null
            }
            dLog { "resultData.size:" + resultData.size + ",sampleCount:" + sampleCount }
            if (resultData.size <= limitDataSize) {
                return resultData
            }
        }
     ...
     */
    private fun compressGifDataWithLongWidth(
        context: Context,
        rawData: ByteArray,
        limitLongWidth: Int
    ): ByteArray? {
        ...
        // 计算调整后大小
        val longSideWidth = header.width.coerceAtLeast(header.height)
     ...
     */
    private fun compressGifDataWithSampleCount(
        context: Context,
        rawData: ByteArray,
        sampleCount: Int
    ): ByteArray? {
        ...
        val frameCount = gifDecoder.frameCount
        dLog { "rawData.size:" + rawData.size + ",frameCount:" + frameCount }
        // 计算帧的间隔
        val frameDurations = (0 until frameCount).map { gifDecoder.getDelay(it) }
        dLog { "rawData.size:" + rawData.size + ",frameDurations:" + frameDurations }
        // 合并帧的时间,最长不可高于 200ms
        val mergeFrameDurations = (0 until frameCount).filter { it % sampleCount == 0 }.map {
            Math.min(
                frameDurations.subList(
                    it,
                    Math.min(it + sampleCount, frameCount)
                ).fold(0) { acc, duration -> acc + duration }, 200
            )
        }
        dLog { "rawData.size:" + rawData.size + ",mergeFrameDurations:" + mergeFrameDurations }
        // 抽取帧
        ...
        dLog { "sampleImageFrames.size:" + sampleImageFrames.size }
        val gifEncoder = AnimatedGifEncoder()
        ...
        dLog { "resultData.size:" + resultData?.size }
        return resultData
    }
}
